...
import com.alekvalek.consecutivepractices.ui.components.RatingBox
import com.alekvalek.consecutivepractices.ui.components.CustomAppBar
...
private fun MovieScreenContent(
    ...
    val fullInnerPadding =
        PaddingValues(end = Spacing.medium, start = Spacing.medium, bottom = Spacing.medium, top = Spacing.small)
    val upLeftInnerPadding =
        PaddingValues(top = Spacing.small, start = Spacing.small)

    Scaffold(
        topBar = { CustomAppBar(state.movie?.name.orEmpty(), onBackPressed) },
        containerColor = MaterialTheme.colorScheme.background
    ) {
        val movie = state.movie ?: run {
            EmptyDataBox(stringResource(R.string.no_results))
            return@Scaffold
        }
                ...
                Card(
                    ...
                    Column {
                        Text(
                            ...
                            modifier = Modifier.padding(upLeftInnerPadding)
                        )
                        Text(
                            text = it,
                            style = MaterialTheme.typography.bodyMedium,
                            modifier = Modifier.padding(fullInnerPadding)
                        )
